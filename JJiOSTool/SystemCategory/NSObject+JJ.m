...
static inline dispatch_time_t jj_dTimeDelay(NSTimeInterval time)
{
    int64_t delta = (int64_t)(NSEC_PER_SEC * time);
    return dispatch_time(DISPATCH_TIME_NOW, delta);
}

BOOL jj_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void jj_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void jj_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
- (id)jj_performBlock:(void (^)(id obj))block onQueue:(dispatch_queue_t)queue afterDelay:(NSTimeInterval)delay
{
    ...
    dispatch_after(jj_dTimeDelay(delay), queue, ^{
        wrapper(NO);
    });
...
+ (id)jj_performBlock:(void (^)(void))block onQueue:(dispatch_queue_t)queue afterDelay:(NSTimeInterval)delay
{
    ...
    dispatch_after(jj_dTimeDelay(delay), queue, ^{ wrapper(NO); });
...
{
    jj_method_append(self.class, klass, newMethod);
}

+ (void)replaceMethod:(SEL)method fromClass:(Class)klass
{
    jj_method_replace(self.class, klass, method);
}
...
+ (u_int32_t)jj_randomWithMaxValue:(u_int32_t)maxValue_ minValue:(u_int32_t)minValue_
{
    return arc4random_uniform(maxValue_ - minValue_) + minValue_;
}

+ (u_int32_t)jj_randomWithMaxValue:(u_int32_t)maxValue_
{
    return [NSObject jj_randomWithMaxValue:maxValue_ minValue:0];
}

+ (double)jj_randomForDouble
{
    srand48(time(nil));
...
+ (CGFloat)jj_colorRandom
{
    return [self jj_random] / (CGFloat)INT_MAX;
}

#pragma mark - swift

+ (Class)jj_swiftClassFromClassName:(NSString *)className_
{
    NSString *appName = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleName"];
    NSString *swiftClassName = [NSString stringWithFormat:@"_TtC%ld%@%ld%@", (long)appName.length, appName, (long)className_.length, className_];
    Class resultClass = NSClassFromString(swiftClassName);
    return resultClass;
}
